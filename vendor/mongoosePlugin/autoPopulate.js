export default function autopopulatePlugin(schema, options) {
  const pathsToPopulate = getPathsToPopulate(schema);

  let testFunction = () => true;
  if (
    options != null &&
    (Array.isArray(options.functions) || options.functions instanceof RegExp)
  ) {
  ...
  const autopopulateHandler = function (filter) {
    ...
    for (let i = 0; i < numPaths; ++i) {
      ...
      pathsToPopulate[i].options.options =
        pathsToPopulate[i].options.options || {};
      ...
      const optionsToUse = processOption.call(
        this,
  ...
  if (testFunction("find")) {
    schema.pre("find", function () {
      return autopopulateHandler.call(this);
    });
    schema.post("find", function (res) {
      return autopopulateDiscriminators.call(this, res);
    });
  }
...
function autopopulateDiscriminators(res) {
  ...
  for (const doc of res) {
    if (doc.constructor.baseModelName != null) {
      ...
      if (!discriminators.has(modelName)) {
        const pathsToPopulate = getPathsToPopulate(
  ...
  return Promise.all(
    Array.from(discriminators.values()).map((modelMap) => {
  ...
  schema.eachPath(function (pathname, schemaType) {
    ...
    if (schemaType.schema) {
      ...
      if (schemaType.schema.discriminators != null) {
        for (const discriminatorName of Object.keys(
          schemaType.schema.discriminators
        )) {
          eachPathRecursive(
